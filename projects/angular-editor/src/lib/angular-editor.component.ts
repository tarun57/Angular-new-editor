...
export class AngularEditorComponent implements OnInit, ControlValueAccessor, AfterViewInit, OnDestroy {
  ...
  onTextAreaBlur(event: FocusEvent) {
     ...
     */
    // Get last position of cursor on blur
    const selection = window.getSelection();
    const range = selection.getRangeAt(0);
    this.lastNode = range.endContainer;
  ...
  // Set last position of cursor on focus
  setCursorToLastPosition() {
    if(this.lastNode && this.lastOffset){
    const range = document.createRange();
    range.setStart(this.lastNode, this.lastOffset);
    range.collapse(true);
